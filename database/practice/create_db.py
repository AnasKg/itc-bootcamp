...
producers_table = '''CREATE TABLE producers(
    producer_id SERIAL PRIMARY KEY,
    producer_country VARCHAR(50),
    producer_company VARCHAR(50),
    produced_date DATE,
    created_at DATE DEFAULT CURRENT_TIMESTAMP
);'''
...
producers_insert = 'INSERT INTO producers (producer_country,producer_company,produced_date) VALUES '
categories_insert = 'INSERT INTO categories (category_name) VALUES '
kivano_insert = 'INSERT INTO kivano (category_id, producer_id, product_name, price) VALUES '
sulpak_insert = 'INSERT INTO sulpak (category_id, producer_id, product_name, price) VALUES '
...
for i in range(SIZE):
    ...
    producers_insert += f'(\'{countries[rdt(0, len(countries)-1)]}\',\'{comp_keys[cmp_n]}\',\'{dates[rdt(0, len(dates)-1)]}\'),'
    ...
    producers_insert = producers_insert[:-1] + ';'
...
cursor.execute(producers_table)
connection.commit()
cursor.execute(kivano_table)
cursor.execute(sulpak_table)
connection.commit()

cursor.execute(producers_insert)
